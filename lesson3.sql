...
DROP TABLE IF EXISTS posts; -- добавлена таблица для хранения постов
CREATE TABLE posts (
	id SERIAL,
...
DROP TABLE IF EXISTS likes; -- задание №2
CREATE TABLE likes (
	id SERIAL,
	from_user_id BIGINT UNSIGNED NOT NULL,
    to_user_id BIGINT UNSIGNED NOT NULL,
    to_media BIGINT UNSIGNED,
    to_post BIGINT UNSIGNED,
    
    PRIMARY KEY (`id`),
	FOREIGN KEY (from_user_id) REFERENCES users(id),
	FOREIGN KEY (to_user_id) REFERENCES users(id),
	FOREIGN KEY (to_media) REFERENCES media(id),
	FOREIGN KEY (to_post) REFERENCES posts(id)
);

ALTER TABLE vk.posts 
ADD CONSTRAINT posts_fk_1
FOREIGN KEY (likes) REFERENCES likes(id);
